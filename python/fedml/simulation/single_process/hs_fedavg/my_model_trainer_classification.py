...
class MyModelTrainer(ClientTrainer):
    ...
    def fft_tensor(self, x: torch.Tensor, amp: torch.Tensor):
        x = x.detach().numpy()
        amp = amp.detach().numpy()
        x, amp = fft(x, amp)
        return torch.from_numpy(x), torch.from_numpy(amp)

    def train(self, train_data, class_weight, device, args, amp: torch.Tensor):
        model = self.model
        model.to(device)
        model.train()
